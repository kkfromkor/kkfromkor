...
# Replay-derived repro. On the failing turn (P1's 2nd turn), the opponent P0's
# LEADER was RESTED (it attacked the prior turn) and P0's characters were
# ACTIVE (untargetable). Question: can P1 declare an attack when its only
# legal target is the opponent's RESTED leader? A probe rests P0's leader and
# activates a P0 character just before P1's turn, then we dump the size of
# P1's attackable block from the core's idle command.
...
public static class RestedLeader {
    ...
    const uint LEADER=880000634; const uint CHAR=880000001; const uint FILLER=880000881;
    static string standardScripts, expansionScripts;
    static readonly List<string> errors=new List<string>(); static readonly List<string> callbackErrors=new List<string>(); static readonly List<string> avail=new List<string>();
    ...
    static int ReadScript(IntPtr payload, IntPtr duel, IntPtr name){ try{ return Load(duel, Marshal.PtrToStringAnsi(name)); } catch(Exception e){ callbackErrors.Add("scr: "+e); return 0; } }
    ...
    public static int Run(string repo){
        ...
        try {
            ...
            // seat P0 char + P1 char active at T1; at start of T4 (P1's turn) REST P0's leader
            // and keep P0's char active -> P1's only legal target is the RESTED P0 leader.
            string probe =
              ...
              "  if tc==4 then local ld=opcg.GetLeader(0) if ld and not opcg.IsRested(ld) then opcg.SetRested(ld) Debug.Message('rested P0 leader, isRested='..tostring(opcg.IsRested(ld))) end\n"+
              "    local g=Duel.GetMatchingGroup(function(x) return opcg.IsLeader(x) or opcg.IsCharacter(x) end,1,LOCATION_MZONE,0,nil)\n"+
              "    for c in aux.Next(g) do Debug.Message('P1atk? code='..c:GetOriginalCode()..' rested='..tostring(opcg.IsRested(c))..' cannotatk='..tostring(c:IsHasEffect(EFFECT_CANNOT_ATTACK)~=nil)) end\n"+
              "    local tg=Duel.GetMatchingGroup(function(x) return true end,0,LOCATION_MZONE,0,nil)\n"+
              "    for c in aux.Next(tg) do Debug.Message('P0target code='..c:GetOriginalCode()..' rested='..tostring(opcg.IsRested(c))..' leader='..tostring(opcg.IsLeader(c))..' char='..tostring(opcg.IsCharacter(c))..' cannotBeTgt='..tostring(c:IsHasEffect(EFFECT_CANNOT_BE_BATTLE_TARGET)~=nil)) end\n"+
              "  end\n"+
            ...
            for(int step=0; step<20000; ++step){
                ...
                if(lastId==11){
                    Reader r=new Reader(lastMsg,1); byte ip=r.U8();
                    uint n;
                    ...
                    r.U8();r.U8();r.U8();
                    uint nAtk=r.U32();
                    avail.Add("T"+newTurns+" P"+ip+" attackable="+nAtk);
                    RespondI32(duel,7);
                }
                ...
                else if(lastId==26){ RespondI32(duel,-1); } else if(lastId==25){ RespondI32(duel,-1); } else if(lastId==19){ RespondI32(duel,0x1); }
        ...
        foreach(string e in errors) Console.WriteLine("PROBE/"+e);
        foreach(string e in errors) if(e.Contains("rested P0")||e.Contains("P1atk")||e.Contains("P0target")) {}
        bool p1t4 = false;
        foreach(string s in avail) if(s.StartsWith("T4 P1") && !s.EndsWith("attackable=0")) p1t4 = true;
        Console.WriteLine(p1t4 ? "P1_T4_VS_RESTED_LEADER: available (OK)" : "P1_T4_VS_RESTED_LEADER: BLOCKED (bug reproduced)");
        return 0;
    }
}
'@
Add-Type -TypeDefinition $source -Language CSharp
[RestedLeader]::LoadDb("$PSScriptRoot\cdb_dump.csv")
exit [RestedLeader]::Run((Resolve-Path -LiteralPath $Repo).Path)
