...
public static class JinbeBattleVerify {
    ...
    public static int Run(string repo) {
        ...
        try {
            ...
            string probeLua =
                ...
                "  local life = Duel.GetFieldGroup(1, LOCATION_EXTRA, 0)\n" +
                "  local parts = {}\n" +
                "  for card in aux.Next(life) do\n" +
                "    parts[#parts + 1] = card:GetSequence() .. ':' .. card:GetOriginalCode()\n" +
                "  end\n" +
                "  table.sort(parts)\n" +
                "  Debug.Message('PROBE turn=' .. turn .. ' p1_life=[' .. table.concat(parts, ' ') .. ']')\n" +
                "end)\n" +
            ...
            addCards(0, LEADER, 1);
            addCards(0, 880000066, 45);
            addCards(1, LEADER, 1);
            addCards(1, (uint)JINBE, 45);
            OCG_StartDuel(duel);
            ...
            for (step = 0; step < 20000; ++step) {
                ...
                while (off + 4 <= all.Length) {
                    ...
                    if (msgId == 1) retries++;
                        ...
                        if (code == JINBE && (ploc != cloc || pcon != ccon)) {
                            string s = "0x" + ploc.ToString("x") + "(p" + pcon + ")->0x" + cloc.ToString("x") + "(p" + ccon + ")";
                            jinbeMoves.Add(s);
                            Console.WriteLine("step " + step + ": JINBE MOVE " + s);
                        }
                    }
                }
                if (status == 0) break;
                ...
                if (newTurns >= 6) break;
                bool jinbeInHand = false;
                foreach (string s in jinbeMoves) if (s.EndsWith("0x2(p1)")) jinbeInHand = true;
                if (jinbeInHand && newTurns >= 3) break;
            }
        ...
        bool sawLimbo = false, sawHand = false, direct = false;
        foreach (string s in jinbeMoves) {
            if (s.StartsWith("0x40") && s.Contains("->0x20")) sawLimbo = true;
            if (s.StartsWith("0x20") && s.Contains("->0x2(")) sawHand = true;
            if (s.StartsWith("0x40") && (s.Contains("->0x2(") || s.Contains("->0x10"))) direct = true;
        }
        Console.WriteLine("trigger_limbo=" + sawLimbo + " limbo_to_hand=" + sawHand + " bypassed_procedure=" + direct);
        bool pass = errors.Count == 0 && callbackErrors.Count == 0 && retries == 0
            && attacked && sawLimbo && sawHand;
        Console.WriteLine(pass ? "JINBE_BATTLE PASS" : "JINBE_BATTLE FAIL");
        return pass ? 0 : 1;
    }
}
'@
...
[JinbeBattleVerify]::LoadDb("$PSScriptRoot\cdb_dump.csv")
exit [JinbeBattleVerify]::Run((Resolve-Path -LiteralPath $Repo).Path)
