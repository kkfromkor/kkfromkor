...
public static class MidattackPlayVerify {
    ...
    public static int Run(string repo) {
        ...
        try {
            ...
            string probeLua =
                ...
                "  if opcg._probe_placed then return end\n" +
            ...
            for (step = 0; step < 20000; ++step) {
                ...
                while (off + 4 <= all.Length) {
                    ...
                    if (msgId == 1) retries++;
                        ...
                        if (code == SNAKE && (ploc != cloc || pcon != ccon)) {
                            string s = "0x" + ploc.ToString("x") + "->0x" + cloc.ToString("x");
                            snakeMoves.Add(s);
                ...
                if (lastMsgId == 13 || lastMsgId == 12) {
                    ...
                    for (uint i = 0; i < n; ++i) {
                        ...
                        if (code == LEADER && desc == 1157 && attackIdx < 0) attackIdx = (int)i;
                    }
                    ...
                    for (uint i = 0; i < n; ++i) {
                        ...
                        if (code == LEADER && loc == 0x4 && seq == 5 && con == 1) pick = (int)i;
                    }
        ...
        bool played = false;
        foreach (string s in snakeMoves) if (s.EndsWith("->0x4")) played = true;
        bool onPlayFired = false, queueDrained = false;
        foreach (string probe in probes) {
            if (probe.Contains("p0_char_power=4000")) onPlayFired = true;
            if (probe.Contains("queue_pending=0") && probe.Contains("direct_pending=0")) queueDrained = true;
        }
        Console.WriteLine("snake_played=" + played + " on_play_fired=" + onPlayFired + " queue_drained=" + queueDrained);
        bool pass = errors.Count == 0 && callbackErrors.Count == 0 && retries == 0
            && attacked && played && onPlayFired && queueDrained;
        Console.WriteLine(pass ? "MIDATTACK_PLAY PASS" : "MIDATTACK_PLAY FAIL");
        return pass ? 0 : 1;
    }
}
'@
...
[MidattackPlayVerify]::LoadDb("$PSScriptRoot\cdb_dump.csv")
exit [MidattackPlayVerify]::Run((Resolve-Path -LiteralPath $Repo).Path)
