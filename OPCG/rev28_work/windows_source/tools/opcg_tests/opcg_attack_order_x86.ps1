...
# 어택 시 효과 창 순서 실측(유저 지시: 선언 → 턴P 어택시 → 비턴P 어택시 → 블로커).
#  P1 리더 = 루시(880002304, 【어택 시】/【상대의 어택 시】 패 버려 펌프)
#  P1 필드 = 블로커(880000005), P1 패 = 이벤트(버릴 재료)
#  P0 캐릭터가 t3에 P1 리더를 어택 → 프롬프트 순서 기록:
#   루시 '상대의 어택 시' 창(YesNo/EffectYN, 비턴P 어택시)이
#   블록 프롬프트(desc 상위 = 879999999)보다 먼저 와야 PASS.
...
public static class AtkOrder {
    ...
    const uint LUCY = 880002304;   // OP15-002 리더(어택 시/상대의 어택 시)
    const uint BLOCK5 = 880000005; // 블로커
    const uint EVCARD = 880000008; // 이벤트(버릴 재료)
    const uint ATTACKER = 880000001; // 바닐라 7000
    const uint LEADER0 = 880000634;
    const uint FILLER = 880000881;
    const ulong BLOCK_DESC_HI = 879999999UL;
    ...
    public static int Run(string repo) {
        ...
        int newTurns = 0; bool attacked = false;
        try {
            ...
            string probe =
              ...
              "  local b=Duel.GetMatchingGroup(function(x) return x:GetOriginalCode()==880000005 end,1,LOCATION_DECK,0,nil):GetFirst()\n" +
              "  if b then Duel.MoveToField(b,1,1,LOCATION_MZONE,POS_FACEUP_ATTACK,true) end\n" +
              "end) Duel.RegisterEffect(pr,0)\n";
            byte[] pb = Encoding.UTF8.GetBytes(probe);
            if (OCG_LoadScript(duel, pb, (uint)pb.Length, "order_probe.lua") != 1) callbackErrors.Add("probe fail");
            ...
            add(1, LUCY, 1); add(1, FILLER, 30); add(1, BLOCK5, 6); add(1, EVCARD, 14);
            OCG_StartDuel(duel);
            ...
            for (int step=0; step<40000; ++step) {
                ...
                if(lastId==11){
                    ...
                    if(idlePlayer==0 && newTurns>=3 && !attacked && atkIdx>=0){
                        attacked=true; order.Add("DECLARE");
                        RespondI32(duel,(atkIdx<<16)|9); continue;
                    }
                    RespondI32(duel,7); continue;
                }
                else if(lastId==12){ // SELECT_EFFECTYN: player, code, loc...
                    Reader r=new Reader(lastMsg,1); byte p2=r.U8(); uint code=r.U32();
                    if(attacked) order.Add("EFFYN p"+p2+" c"+code);
                    RespondI32(duel,0);
                }
                else if(lastId==13){ // SELECT_YESNO: player, desc u64
                    Reader r=new Reader(lastMsg,1); byte p2=r.U8(); ulong desc=r.U64();
                    ulong hi=desc>>20;
                    if(attacked) order.Add((hi==BLOCK_DESC_HI?"BLOCKPROMPT":"YESNO")+" p"+p2+" d"+hi);
                    RespondI32(duel,0);
                }
                else if(lastId==14){ Reader r=new Reader(lastMsg,1); byte p2=r.U8(); if(attacked) order.Add("OPT p"+p2); RespondI32(duel,0); }
                else if(lastId==15){
                    Reader r=new Reader(lastMsg,1); byte p2=r.U8(); r.U8();
                    uint mn=r.U32(); uint mx=r.U32(); uint n=r.U32();
                    uint c0 = n>0 ? BitConverter.ToUInt32(lastMsg, 11+4) : 0;
                    if(attacked) order.Add("SEL p"+p2+" n"+n+" min"+mn+" c"+c0);
        ...
        Console.WriteLine("--- order after declare ---");
        foreach(var s2 in order) Console.WriteLine(s2);
        int lucyIdx=-1, blockIdx=-1;
        for(int i2=0;i2<order.Count;++i2){
            string s2=order[i2];
            if(lucyIdx<0 && s2.StartsWith("SEL p1") && !s2.Contains("c"+BLOCK5)) lucyIdx=i2;
            if(lucyIdx<0 && (s2.StartsWith("EFFYN p1")||s2.StartsWith("YESNO p1"))) lucyIdx=i2;
            if(blockIdx<0 && s2.StartsWith("BLOCKPROMPT")) blockIdx=i2;
        }
        Console.WriteLine("lucyIdx="+lucyIdx+" blockIdx="+blockIdx);
        bool pass = lucyIdx>=0 && blockIdx>=0 && lucyIdx<blockIdx && errors.Count==0;
        Console.WriteLine(pass ? "ATTACK_ORDER PASS" : "ATTACK_ORDER FAIL");
        return pass ? 0 : 1;
    }
}
'@
...
exit [AtkOrder]::Run((Resolve-Path -LiteralPath $Repo).Path)
