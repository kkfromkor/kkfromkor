...
# P-117 Nami leader special win (user report 2026-07-27: does not fire):
# [Rule] if your deck hits 0 cards you WIN instead of losing. This drives a
# real duel with P-117 seated as P0's leader, trashes P0's whole deck at T2
# main, and expects MSG_WIN (5) declaring player 0 the winner.
...
public static class DeckoutWin {
    ...
    const uint NAMI117 = 880002073; // P-117 special-win leader (P0)
    const uint LEADER  = 880000634; // plain leader (P1)
    ...
    public static int Run(string repo) {
        ...
        try {
            ...
            string probeLua =
                ...
                "  if Duel.GetTurnCount() ~= 2 or opcg._dw_setup then return end\n" +
                "  opcg._dw_setup = true\n" +
                "  local lead = opcg.GetLeader(0)\n" +
                "  Debug.Message('leader='..tostring(lead and lead:GetOriginalCode()))\n" +
                "  Debug.Message('cannot_lose='..tostring(Duel.IsPlayerAffectedByEffect(0, EFFECT_CANNOT_LOSE_DECK) ~= nil))\n" +
                "  local deck = Duel.GetFieldGroup(0, LOCATION_DECK, 0)\n" +
                "  Debug.Message('deck_before='..deck:GetCount())\n" +
                "  Duel.SendtoGrave(deck, REASON_RULE)\n" +
                "  Debug.Message('deck_after='..Duel.GetFieldGroupCount(0, LOCATION_DECK, 0))\n" +
                "end)\n" +
            ...
            add(0, NAMI117, 1); add(0, FILLER, 45);
            ...
            for (int step = 0; step < 20000; ++step) {
                ...
                while (off + 4 <= all.Length) {
                    ...
                    if (id == 40) newTurns++;
                    else if (id == 5 && pl2 >= 2) { winPlayer = payload[1]; Console.WriteLine("MSG_WIN player=" + winPlayer + " (T" + newTurns + ")"); }
                }
                if (winPlayer >= 0) break;
                if (status == 0) { Console.WriteLine("duel ended, status=0"); break; }
        ...
        Console.WriteLine("win_player=" + winPlayer);
        bool pass = winPlayer == 0 && errors.Count == 0 && callbackErrors.Count == 0;
        Console.WriteLine(pass ? "DECKOUT_WIN PASS" : "DECKOUT_WIN FAIL");
        return pass ? 0 : 1;
    }
}
'@
...
[DeckoutWin]::LoadDb("$PSScriptRoot\cdb_dump.csv")
exit [DeckoutWin]::Run((Resolve-Path -LiteralPath $Repo).Path)
