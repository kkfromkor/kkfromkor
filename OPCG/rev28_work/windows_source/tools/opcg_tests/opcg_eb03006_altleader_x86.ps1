...
public static class Eb03006AltLeader {
    ...
    const uint VIVI   = 881000703; // ALT-PRINT Vivi leader (cdb alias -> 880002104): the 2026-07-27 replay bug
    ...
    public static int Run(string repo) {
        ...
        bool e2ActivatedT7 = false; bool t7Reported = false;
        try {
        ...
        bool pass = played && leadDropped && e2Activated && vicDropped && drawT5 >= 2 && e2ActivatedT7 && vicT7
            && errors.Count == 0 && callbackErrors.Count == 0;
        Console.WriteLine(pass ? "EB03006_ALTLEADER PASS" : "EB03006_ALTLEADER FAIL");
        return pass ? 0 : 1;
    }
}
'@
...
[Eb03006AltLeader]::LoadDb("$PSScriptRoot\cdb_dump.csv")
exit [Eb03006AltLeader]::Run((Resolve-Path -LiteralPath $Repo).Path)
