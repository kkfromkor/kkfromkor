...
# 4-cost character (880000002, power below 7000) at T3 -> the core's battle
...
public static class BattleReplKo {
    ...
    const uint VICTIM   = 880000002; // base cost 4, power < 7000, no blocker
    ...
    public static int Run(string repo) {
        ...
        try {
            ...
            string probeLua =
                ...
                "    if vic then Duel.MoveToField(vic,1,1,LOCATION_MZONE,POS_FACEUP_DEFENSE,true) Debug.Message('placed victim rested='..tostring(opcg.IsRested(vic))) end\n" +
        ...
        Console.WriteLine("attacked=" + attacked + " registered=" + registered
        ...
        bool pass = attacked && battlePrompt && battleSaved && costPaidT3 && effectSaved
...
[BattleReplKo]::LoadDb("$PSScriptRoot\cdb_dump.csv")
exit [BattleReplKo]::Run((Resolve-Path -LiteralPath $Repo).Path)
