...
public static class RedirectAttack {
    ...
    const uint DOFLA    = 880002225; // OP14-060 leader (P1)
    ...
    public static int Run(string repo) {
        ...
        bool attacked = false, chainFired = false, redirected = false, baby5Died = false, targetPicked = false;
        int attackMsgs = 0;
        ...
        try {
            ...
            add(1, DOFLA, 1); add(1, BABY5, 1); add(1, FILLER, 44);
            OCG_StartDuel(duel);
            ...
            for (int step = 0; step < 20000; ++step) {
                ...
                if (lastId == 12) { // effect yes/no: YES for the Doflamingo prompt
                    ...
                    if (code == DOFLA && newTurns == 3) { chainFired = true; Console.WriteLine("T3 DOFLA PROMPT(12) -> YES"); RespondI32(duel, 1); }
                    ...
                    for (uint i = 0; i < n; ++i) { uint code = r.U32(); r.U8(); r.U8(); r.U32(); r.U32(); r.U64(); r.U8(); if (code == DOFLA && doflaIdx < 0) doflaIdx = (int)i; }
                    if (doflaIdx >= 0 && newTurns == 3 && !chainFired) { chainFired = true; Console.WriteLine("T3 DOFLA CHAIN -> activate"); RespondI32(duel, doflaIdx); }
                    ...
                    if (b5Idx >= 0) { targetPicked = true; Console.WriteLine("select -> BABY5"); resp.AddRange(BitConverter.GetBytes((uint)b5Idx)); }
        ...
        Console.WriteLine("attacked=" + attacked + " effect_fired=" + chainFired + " target_picked=" + targetPicked
            + " attack_msgs=" + attackMsgs + " redirected=" + redirected + " baby5_died=" + baby5Died);
        bool pass = attacked && chainFired && targetPicked && redirected && baby5Died
            && errors.Count == 0 && callbackErrors.Count == 0;
        Console.WriteLine(pass ? "REDIRECT_ATTACK PASS" : "REDIRECT_ATTACK FAIL");
        return pass ? 0 : 1;
    }
}
'@
...
[RedirectAttack]::LoadDb("$PSScriptRoot\cdb_dump.csv")
exit [RedirectAttack]::Run((Resolve-Path -LiteralPath $Repo).Path)
