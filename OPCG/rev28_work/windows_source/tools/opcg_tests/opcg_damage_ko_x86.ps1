...
public static class KoBattleVerify {
    ...
    public static int Run(string repo) {
        ...
        try {
            ...
            string probeLua =
                ...
                "      return c:GetOriginalCode() == 880000066 end, 1, LOCATION_DECK + LOCATION_HAND, 0, nil):GetFirst()\n" +
                "    if card then\n" +
                "      Duel.MoveToField(card, 1, 1, LOCATION_MZONE, POS_FACEUP_ATTACK, true)\n" +
                "      opcg.SetRested(card)\n" +
                "      Debug.Message('PROBE placed rested 880000066 seq=' .. card:GetSequence())\n" +
                "    end\n" +
                "  end\n" +
                "end)\n" +
            ...
            for (step = 0; step < 20000; ++step) {
                ...
                while (off + 4 <= all.Length) {
                    ...
                    if (msgId == 1) retries++;
                        ...
                        if ((code == JINBE || code == 880000066) && (ploc != cloc || pcon != ccon)) {
                ...
                if (lastMsgId == 13 || lastMsgId == 12) {
                    ...
                    for (uint i = 0; i < n; ++i) {
                        ...
                        if (code == 880000066 && loc == 0x4 && con == 1) pick = (int)i;
                    }
                ...
                if (newTurns >= 6) break;
                bool koSeen = false;
                foreach (string s in jinbeMoves) if (s.StartsWith("880000066") && s.EndsWith("0x10(p1)")) koSeen = true;
        ...
        foreach (string s in jinbeMoves) {
            if (s.StartsWith("880000066") && s.Contains("0x4(p1)->0x10(p1)")) koToTrash = true;
        }
        Console.WriteLine("ko_to_trash=" + koToTrash);
        bool pass = errors.Count == 0 && callbackErrors.Count == 0 && retries == 0
            && attacked && koToTrash;
        Console.WriteLine(pass ? "KO_BATTLE PASS" : "KO_BATTLE FAIL");
        return pass ? 0 : 1;
    }
}
'@
...
[KoBattleVerify]::LoadDb("$PSScriptRoot\cdb_dump.csv")
exit [KoBattleVerify]::Run((Resolve-Path -LiteralPath $Repo).Path)
